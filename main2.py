...
def main():
    ...
    distance_threshold = 50  # Mesafe eşiği (piksellerde)
    ...
    # Kafa tespiti için Haar cascade dosyası yükleyin
    head_cascade = cv2.CascadeClassifier(cv2.data.haarcascades + 'default.xml')
        ...
        if frame_count % (update_interval * fps) == 0:  # Her saniyede bir kare al ve güncelle
            ...
            for result in results:
                for detection in result.boxes:
                    ...
                    bbox = detection.xyxy  # x_min, y_min, x_max, y_max

                    if class_name == "baret":
                        detection_state.baret_last_detected = current_time
                        detection_state.helmet_position = ((bbox[0] + bbox[2]) / 2, (bbox[1] + bbox[3]) / 2)
            ...
            for (x, y, w, h) in heads:
                ...
                detection_state.head_position = (x + w / 2, y + h / 2)
                # Kafa tespitini çizin
                cv2.rectangle(frame, (x, y), (x + w, y + h), (255, 0, 0), 2)
                break  # İlk tespit edilen kafayı kullan

            if detection_state.head_position and detection_state.helmet_position:
                distance = calculate_distance(detection_state.head_position, detection_state.helmet_position)
                if distance > distance_threshold:
                    detection_state.baret_last_detected = 0  # Baret durumunu false yap
